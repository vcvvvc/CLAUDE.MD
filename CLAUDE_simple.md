...
- **Code Delivery**: Enable this gate only when the current turn involves code development, code modification, script generation, test writing, or engineering configuration changes; pure Q&A, code explanation, solution discussion, requirement clarification, and document polishing are not constrained by this item. Any request containing multiple features, multiple files, or "from scratch" creation must be split first; the current turn may implement only one minimum verifiable slice, and the slice may have only one acceptance target. If the slice description includes a happy path, persistence, initialization, multiple error types, read/write flow, or multiple commands at the same time, it must be split further. Each code-logic change may deliver only one small function or must be fewer than `35` lines. Documentation, configuration, styling, and tests are treated as later slices by default; they may be changed in the same turn only when the user explicitly asks for them "in this turn", or when the current code slice cannot be verified with existing commands, syntax checks, one CLI run, or Code Trace; they must still keep the smallest reviewable diff. Do not generate large files in one pass; do not write, output, or scaffold code for later steps as part of the current step. After completing the current slice, stop and wait for explicit user authorization to continue.
...
## 7. Final Response Review
- [ ] **Plan Check**: If the user requested a development plan, was `_PLAN.md` created and strictly followed?
- [ ] **Cadence Check**: If this turn involved code development, code modification, script generation, test writing, or engineering configuration changes, was the minimum verifiable slice declared and followed? Did the slice have only one acceptance target, without combining the happy path, persistence, initialization, multiple error types, read/write flow, or multiple commands in the same turn? Did the code-logic change satisfy both conditions: only one single-responsibility small function was changed/added, and the logic diff was fewer than `35` lines? Did the change avoid bypassing the slice limit by stuffing a multi-step flow into one function? Did it avoid adding interfaces, state, configuration, tests, documentation, placeholders, or next-step implementation that the current slice does not need for verification?
