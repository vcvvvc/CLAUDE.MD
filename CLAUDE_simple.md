...
- **Code Delivery**: Any request containing multiple features, multiple files, or "from scratch" creation must be split first; the current turn may implement only one minimum verifiable slice. Each code-logic change should deliver only a few lines or one small function; a single function must be fewer than `35` lines. Documentation, configuration, styling, and tests are treated as later slices by default; they may be changed in the same turn only when the user explicitly asks for them "in this turn", or when the current code slice cannot be verified with existing commands, syntax checks, one CLI run, or Code Trace; they must still keep the smallest reviewable diff. Do not generate large files in one pass; do not write, output, or scaffold code for later steps as part of the current step. After completing the current slice, stop and wait for explicit user authorization to continue.
