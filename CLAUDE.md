...
- **Bilingual Separation**:
  - **Thought Process / Tool Calls (CoT)**: Use **English** only to ensure reasoning depth.
  - **Final Output**: Use **Chinese** only to ensure precision of expression.
...
- **First Principles**: Start from the essence of the problem. Identify the **goal, constraints, and failure modes** first, then choose the **minimal viable solution**. Do not copy patterns, legacy implementations, or superficially similar solutions.
...
2. If relevant MCP / Diagnostics / Memory tools are currently unavailable, the blocking reason must be stated explicitly. Never pretend the step was executed.

Before any information retrieval, the following routing must be matched strictly:

| Scenario | Action | Constraint & Why |
| :--- | :--- | :--- |
| **0. Session start / retrospective** | Use **Memory MCP** | Read background at session start; write core decisions at session end. |
| **1. Known identifier / function name** | Use **Grep (ripgrep)** | `-C` must be used to read surrounding snippets (scalpel mode). Reading the whole file directly is forbidden. |
...
| **3. Confirm API signature / docs** | Use **Context7** | Official documentation is more reliable than external search for eliminating syntax uncertainty. |
...
1. **Init**: Create the **only** temporary file `_PLAN.md` in the project directory (containing Context conclusions and checklist `[ ]`).
2. **Execute & Halt**:
   - Each time, strictly select exactly **1** minimum-granularity task from the checklist.
   - After completion, mark the item as `[x]` and **immediately stop** all code generation.
   - Report the current change to the user, and explicitly output: **"Please review the current changes. After confirming they are correct, please reply 'continue'."**.
   - Without explicit textual authorization from the user (e.g., "continue"), entering the next `[ ]` item is **absolutely forbidden**.
3. **Audit**: After task completion, **keep** `_PLAN.md` for review. Do not delete it automatically. Do not create redundant files such as findings/progress notes.
...
- If diagnostics tools are unavailable, this must be stated explicitly. Never claim “checked and passed.”
...
- If existing errors unrelated to the current change are detected, they must be reported truthfully, but must not be used as a reason to expand current task scope.
- If three consecutive attempts to fix the same error still fail, further trial-and-error must stop. Report the situation to the user and request confirmation on the next step.

## 6. Consolidation and Closure

**Trigger**: Task completion / a deep pitfall is discovered and resolved.
