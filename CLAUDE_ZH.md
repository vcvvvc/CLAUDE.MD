...
- **交付节奏 (Delivery Cadence)**：
  - **原子逻辑变更 (Atomic Logic Change)**：每次交付必须是一个**逻辑完备、可独立审查的最小单元**（如 1 个核心函数 / 1 个数据结构 / 1 个独立缺陷修复）。禁止强行拆分；允许为完成单一原子任务一次性生成完整文件，但不得扩散当前目标范围。
  - **单修改单元原则 (Single Change Unit)**：每次执行只推进 1 个可独立审查的逻辑变更单元；若改动同时涉及多个彼此独立的行为或规则，必须拆分。判断标准是审查目标，不是文件数。
