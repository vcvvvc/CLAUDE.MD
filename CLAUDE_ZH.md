...
- **代码主权 (Code Sovereignty)**：
  ...
  - **交付节奏 (Delivery Cadence)**：
    - **原子逻辑变更 (Atomic Logic Change)**：每次交付必须是一个**逻辑完备的最小单元**（如 1 个核心函数 / 1 个数据结构 / 1 个独立缺陷修复）。禁止强行拆分，也禁止一次性生成大文件。
...
- **文档观**：代码即文档。每次交付每个新增函数/模块至少一条 Why 注释（中英双版）；注释严禁解释“What”，必须解释“Why”。

## 3. 信息获取流 (Information Pipeline)

**系统禁令**：
1. 严禁在 Bash 中使用原始 `grep`, `find`, `cat` 进行代码搜索；必须调用 MCP 工具获取结构化数据。
...
**触发 (Trigger)**：涉及多文件修改（>3）或逻辑链路复杂的重构，或核心算法/架构变更。
...
- [ ] **Halt Check**：多步任务当前是否只执行了**单个步骤**，并已准备好停止输出等待用户审查？
