...
- **交付节奏 (Delivery Cadence)**：
  - **原子逻辑变更 (Atomic Logic Change)**：每次交付必须是一个**逻辑完备、可独立审查的最小单元**（如 1 个核心函数 / 1 个数据结构 / 1 个独立缺陷修复）。禁止强行拆分，也禁止一次性生成大文件。
  - **单修改单元原则 (Single Change Unit)**：每次执行只允许推进 **1 个**小变更单元（例如一个函数、一个分支、一个小单元），不得在同一步引入多个独立决策点。判断标准是**审查单元**，不是文件数。
  - **禁止预测 (No Lookahead)**：严禁在当前步骤中连带编写或输出下一步骤的代码。
- **防御策略 (Defensive)**：
  - **Public API**：假定输入不可信，必须校验 (Check & Error Handle)。
  - **Internal**：假定状态可信，必须断言 (Assert only)。
- **文档观**：代码即文档。注释只解释 **Why**，不解释显而易见的 **What**；优先在关键决策点、边界处理、兼容性权衡处补充 Why 注释。
...
**触发 (Trigger)**：仅当用户**明确要求**生成 `_PLAN.md` 时触发。

**复杂任务默认行为**：当任务明显复杂，但用户未明确要求 `_PLAN.md` 时，代理应仅提示该任务可能需要计划式推进，**不得自动触发**、不得自行创建 `_PLAN.md`。
...
**触发 (Trigger)**：每次完成代码编辑后，**必须**立即执行静态检查。
...
- **逃生通道 (Escape Hatch)**：若连续尝试修复 **3 次**仍未解决同一 Error，必须中断尝试，向用户汇报报错详情并请求人工介入判定方向。

## 6. 沉淀与闭环 (Consolidation)

**触发 (Trigger)**：任务结束 / 发现并解决深坑。
